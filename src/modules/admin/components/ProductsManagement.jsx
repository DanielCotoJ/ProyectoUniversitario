    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <fieldset className="border border-slate-300 rounded-lg p-3">
          ...
          <Input id="categoria" className="border-none focus:ring-0 px-0" value={formData.categoria} onChange={(e) => setFormData({ ...formData, categoria: e.target.value })} required />
        </fieldset>
      </div>

      <fieldset className="border border-slate-300 rounded-lg p-3">
        <legend className="text-xs text-slate-600 px-2">Descripcion *</legend>
        <Textarea id="descripcion" className="border-none focus:ring-0 px-0" value={formData.descripcion} onChange={(e) => setFormData({ ...formData, descripcion: e.target.value })} required rows={3} />
      </fieldset>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <fieldset className="border border-slate-300 rounded-lg p-3">
          <legend className="text-xs text-slate-600 px-2">Precio *</legend>
          <Input id="precio" type="number" className="border-none focus:ring-0 px-0" value={formData.precio} onChange={(e) => setFormData({ ...formData, precio: Number(e.target.value) })} required min="0" />
        </fieldset>
        <fieldset className="border border-slate-300 rounded-lg p-3">
          <legend className="text-xs text-slate-600 px-2">Stock disponible *</legend>
          <Input id="stock" type="number" className="border-none focus:ring-0 px-0" value={formData.stock} onChange={(e) => setFormData({ ...formData, stock: Number(e.target.value) })} required min="0" />
        </fieldset>
      </div>

      <fieldset className="border border-slate-300 rounded-lg p-3">
        <legend className="text-xs text-slate-600 px-2">Proveedor *</legend>
        <Input id="proveedor" className="border-none focus:ring-0 px-0" value={formData.proveedor} onChange={(e) => setFormData({ ...formData, proveedor: e.target.value })} required />
      </fieldset>

      <fieldset className="border border-slate-300 rounded-lg p-3">
        <legend className="text-xs text-slate-600 px-2">URL de la imagen</legend>
        <Input id="imagen" type="url" className="border-none focus:ring-0 px-0" value={formData.imagen} onChange={(e) => setFormData({ ...formData, imagen: e.target.value })} placeholder="https://ejemplo.com/imagen.jpg" />
      </fieldset>
      ...
      <div className="flex justify-center gap-4 pt-6">
        ...
        <Button type="submit" className="bg-brand-dark hover:bg-brand rounded-full px-6 shadow-sm">{isNew ? "Registrar Producto" : "Guardar cambios"}</Button>
      </div>
    </form>
  )
}
      ...
      <div className="px-4 md:px-6 py-4 border-b border-slate-200 flex items-center justify-between flex-col md:flex-row gap-4 md:gap-0">
        ...
        <Dialog open={isAddingNew} onOpenChange={setIsAddingNew}>
          ...
          <DialogContent className="max-w-[95vw] sm:max-w-xl md:max-w-2xl md:p-8">
            ...
            <ProductForm
              product={{ id: "", nombre: "", descripcion: "", precio: 0, stock: 0, categoria: "", proveedor: "", imagen: "", estado: "Activo" }}
          ...
          <TableBody>
            {products.map((product) => (
              <TableRow key={product.id}>
                ...
                <TableCell>
                  <span className={`inline-flex items-center px-2 py-1 rounded text-xs ${product.estado === "Activo" ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>{product.estado}</span>
                </TableCell>
                <TableCell>
                  <div className="flex gap-2">
                    <Dialog open={Boolean(editingProduct)} onOpenChange={(open) => { if (!open) setEditingProduct(null) }}>
                      <DialogTrigger asChild>
                        <Button variant="outline" size="sm" onClick={() => setEditingProduct(product)} className="text-xs">Editar</Button>
                      </DialogTrigger>
                      <DialogContent className="max-w-[95vw] sm:max-w-xl md:max-w-2xl md:p-8">
                        <DialogHeader>
                    ...
                    {product.estado === "Activo" && (
                      <Button variant="outline" size="sm" onClick={() => handleDeleteProduct(product.id)} className="text-xs text-red-600 hover:text-red-700">Dar de baja</Button>
                    )}
                  </div>
                </TableCell>
              </TableRow>
