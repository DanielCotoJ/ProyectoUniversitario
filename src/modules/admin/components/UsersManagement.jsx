...
import { updateUserFromAdmin } from "@/modules/auth/services/AuthService";
...
export default function UsersManagement({ users, setUsers }) {
  ...
  const handleUpdateUser = (updatedUser) => {
    ...
    // 2) Persistir en el mismo localStorage que usa AuthService
    const { success, message } = updateUserFromAdmin(updatedUser);
    if (!success) {
      console.error("Error guardando usuario en storage:", message);
      // opcional: mostrar toast o revertir cambios
    }
    ...
    <div className="bg-white rounded-lg border border-slate-200">
      <div className="px-4 md:px-6 py-4 border-b border-slate-200">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Visualiza y gestiona la información de los usuarios registrados
        </p>
      </div>
                    ...
                    <DialogContent className="max-w-[95vw] sm:max-w-xl md:max-w-2xl md:p-8">
                      <DialogHeader>
                        ...
                        <DialogDescription className="text-sm md:text-base text-slate-600">
                          Modifica la información del usuario. El ID y email no
