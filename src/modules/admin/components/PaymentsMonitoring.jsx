...
export default function PaymentsMonitoring({ sales }) {
  const paymentsByMethod = sales.reduce((acc, sale) => { acc[sale.metodoPago] = (acc[sale.metodoPago] || 0) + 1; return acc }, {})
  const paymentsByStatus = sales.reduce((acc, sale) => { acc[sale.estadoPago] = (acc[sale.estadoPago] || 0) + 1; return acc }, {})
            ...
            <TableBody>
              {sales.map((sale) => (
                <TableRow key={sale.id}>
                  ...
                  <TableCell>
                    <span className="inline-flex items-center px-2 py-1 rounded text-xs bg-gray-100 text-gray-800">{sale.metodoPago}</span>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Dialog open={Boolean(detailSale)} onOpenChange={(open) => { if (!open) setDetailSale(null) }}>
                      <DialogTrigger asChild>
                        <Button variant="outline" size="sm" className="text-xs bg-transparent" onClick={() => setDetailSale(sale)}>Ver detalles</Button>
                      </DialogTrigger>
                      <DialogContent className="max-w-[95vw] sm:max-w-xl md:max-w-2xl md:p-8">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex justify-center gap-4 pt-6">
                            <Button type="button" variant="outline" className="rounded-full px-6 border-brand-dark text-brand-dark" onClick={() => setDetailSale(null)}>Cerrar</Button>
                            <Button type="button" className="bg-brand-dark hover:bg-brand rounded-full px-6 shadow-sm">Aceptar</Button>
                          </div>
                        </div>
                      </DialogContent>
                    </Dialog>
